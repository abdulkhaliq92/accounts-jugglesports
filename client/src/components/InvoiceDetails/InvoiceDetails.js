...
const printDocument = () => {
  ...
  html2canvas(input, { backgroundColor: "#ffffff" })  // Set backgroundColor to white
    .then((canvas) => {
      ...
      const imgData = canvas.toDataURL('image/jpeg', 1.0); // Keep high quality
...
const downloadDesktopPDF = () =>{
  const input = document.getElementById('mimic-desktop');
  html2canvas(input, { backgroundColor: "#ffffff" , scale: 1.1, pagesplit: true})  // Set backgroundColor to white
    .then((canvas) => {
  ...
  return (
    ...
    return (
      ...
      {/* Modal for mobile view */}
          <Dialog open={openModal} onClose={() => setOpenModal(false)} id='mobile-header'>
            <div className={styles.modalContent} id='mobile-modal'>
              {/* Download button for desktop content */}
              <Button onClick={downloadDesktopPDF} className={styles.btn}>
                Download
              </Button>

              {/* Desktop content */}
              <div id='mimic-desktop'>
              <Container>
              <Grid container justifyContent="space-between" style={{padding: '30px 0px' }} id='logo'>
                <center><Logo width={'950px'} /></center>
              </Grid>
              <Divider />
              <Container>
                      ...
                      <Grid item>
                          {invoice?.creator?.includes(user?.result._id) && (
                            <Container style={{marginBottom: '20px'}}>
                              ...
                              <Typography variant="body2">+92-347-4931430</Typography>
                              <Typography variant="body2">+92-300-6197746</Typography>
          ...
          {invoiceData?.items?.map((itemField, index) => (
           ...
           {invoice?.creator?.includes(user?.result?._id || user?.result?.googleId) && (
            <div className={styles.buttons} id='top-buttons'>
                ...
                <button onClick={printDiv} className={styles.btn} id="generate-pdf-desktop">Download PDF</button>
                {/* <button onClick={printDocument} className={styles.btn} >Download PDF Mobile</button> */}
                <button onClick={() => setOpenModal(true)} className={styles.btn} id="generate-pdf-mobile">
                  Generate PDF
                </button>
                <button 
