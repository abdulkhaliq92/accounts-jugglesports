...
//                 <TableCell  style={tableStyle} onClick={() => openInvoice(row._id)} > {row?.client?.name ?? 'N/A'} </TableCell>
          ...
          <tbody>
            ...
            {pageData.map((r) => (
              <tr key={r._id} onClick={() => openInvoice(r._id)}>
                <td data-label="#">{r.invoiceNumber ?? "—"}</td>
        ...
        {pageData.map((r) => (
          <article key={r._id} className="card" onClick={() => openInvoice(r._id)}>
            <header className="card-head">
              <div className="card-title">{r.invoiceNumber ?? "—"}</div>
